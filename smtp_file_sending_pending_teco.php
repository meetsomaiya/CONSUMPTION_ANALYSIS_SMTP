...
$existingOrderQuery = "SELECT [Order No] FROM [NewDatabase].[dbo].[VW_reason_for_dispute_and_pending_teco]";
...
foreach ($pendingTecoTables as $table) {
    ...
    // Include all rows, regardless of whether the order number exists
foreach ($fetchedRows as $row) {
    $pendingTecoRows[] = $row; // Include all rows
}
...
foreach ($pendingTecoRows as $row) {
    ...
    if (!isset($groupedData[$key])) {
        $groupedData[$key] = [
            ...
            'statePmo' => $statePmo,
            'extra' => $siteMapping[$row['Site']]['extra'] ?? null, // Fetch 'extra' column
