...
// // Fetch today's date
// $currentDate007 = date('Y-m-d'); // $currentDate007 stores today's date
// $currentYear007 = date('Y'); // $currentYear007 stores the current year
// $currentMonth007 = date('n'); // $currentMonth007 stores the numeric month (1-12)

// // Determine financial year
// if ($currentMonth007 >= 3) {
//     $financialYearStart007 = "{$currentYear007}-03-31"; // $financialYearStart007 stores the start date of the financial year
//     $financialYearEnd007 = ($currentYear007 + 1) . "-04-01"; // $financialYearEnd007 stores the end date of the financial year
//     $financialYearLabel007 = "{$currentYear007}-" . ($currentYear007 + 1); // $financialYearLabel007 stores the financial year label
// } else {
//     $financialYearStart007 = ($currentYear007 - 1) . "-03-31"; // $financialYearStart007 stores the start date of the financial year
//     $financialYearEnd007 = "{$currentYear007}-04-01"; // $financialYearEnd007 stores the end date of the financial year
//     $financialYearLabel007 = ($currentYear007 - 1) . "-{$currentYear007}"; // $financialYearLabel007 stores the financial year label
// }

// Fetch today's date
global $financialYearStart007, $financialYearEnd007, $financialYearLabel007;
...
// Determine financial year
if ($currentMonth007 >= 3) {
    $financialYearStart007 = "{$currentYear007}-03-31"; // Start of the financial year
    $financialYearEnd007 = ($currentYear007 + 1) . "-04-01"; // End of the financial year
    $financialYearLabel007 = "{$currentYear007}-" . ($currentYear007 + 1); // Financial year label
} else {
    $financialYearStart007 = ($currentYear007 - 1) . "-03-31"; // Start of the financial year
    $financialYearEnd007 = "{$currentYear007}-04-01"; // End of the financial year
    $financialYearLabel007 = ($currentYear007 - 1) . "-{$currentYear007}"; // Financial year label
}
...
//         WHERE d.[Site] = :site 
//           AND d.[Order] = :orderType
//           AND d.[Posting Date] BETWEEN '{$financialYearStart007}' AND '{$financialYearEnd007}'
//           AND d.[Order No] NOT IN (
//               SELECT vw.[Order No]
//               FROM [VW_reason_for_dispute_and_pending_teco] vw
//           )";
...
function fetchDisputeCount($db, $site, $orderType, $financialYearStart007, $financialYearEnd007) {
    $sql = "
        SELECT COUNT(*) AS order_count
        FROM dispute d
        WHERE d.[Site] = :site 
          AND d.[Order] = :orderType
          AND d.[Posting Date] BETWEEN :startDate AND :endDate
          AND d.[Order No] NOT IN (
              SELECT vw.[Order No]
              FROM [VW_reason_for_dispute_and_pending_teco] vw
          )";
    ...
    $stmt->execute([
        ':site' => $site,
        ':orderType' => $orderType,
        ':startDate' => $financialYearStart007,
        ':endDate' => $financialYearEnd007
    ]);
    return $stmt->fetchColumn();
}
...
FROM [dbo].[VW_site_area_incharge_mapping]";
    ...
    foreach ($inchargeCombinations as $incharge) {
        ...
        // Initialize counts for each oil change type
$fcCount = fetchDisputeCount($db, $site, 'FC_OIL_CHANGE ORDER', $financialYearStart007, $financialYearEnd007);
$gbCount = fetchDisputeCount($db, $site, 'GB_OIL_CHANGE ORDER', $financialYearStart007, $financialYearEnd007);
$pdCount = fetchDisputeCount($db, $site, 'PD_OIL_CHG_ORDER', $financialYearStart007, $financialYearEnd007);
$ydCount = fetchDisputeCount($db, $site, 'YD_OIL_CHG_ORDER', $financialYearStart007, $financialYearEnd007);
...
$sheet->setCellValue("P{$row}", $grandTotal);
    ...
    $fileName = "Dispute_Report_{$financialYearLabel007}.xlsx";
...
$subject = "CONSUMPTION ANALYSIS FY {$financialYearLabel007} - GB FC TILL {$previousMonth} - CLOSE IT BEFORE {$currentDay} {$currentMonth} - SITE FEEDBACK PENDING {$totalGrandOrders} CONSUMPTION ORDER";
...
Please find the attached file contains the consumption details of Gear box, Fluid coupling oil till {$previousMonth}.<br><br>
Kindly provide site justification of order where the oil return is less than 80%, only oil issue, only oil return before <strong>{$currentDay} {$currentMonth}</strong>.<br><br>
This is to inform you that I have highlighted the summary of GB FC oil issue vs oil return as per oil change order for the current financial year {$financialYearLabel007}.<br><br>
The attached file contains order-wise details available with movement type oil issue & oil return for GB, FC.<br>
Those states that have not achieved the oil return target kindly provide the order-wise justification.<br><br>
<strong>ACTION TAKEN BY SITE</strong><br>
Please check the data of your state, site, and location by clicking on the number of b/d or oil change in the summary sheet.<br>
