...
//         WHERE [Site] = :site 
//         AND [Order] = :orderType
//         AND [Posting Date] BETWEEN '2024-04-01' AND '2025-03-31'";
...
// Helper function for fetching counts from a specific table
function fetchCount($db, $site, $table, $orderType, $useOrderStatus = true, $fyStart, $fyEnd) {
    $sql = "
        SELECT COUNT(*) AS order_count
        FROM $table
        WHERE [Site] = :site 
        AND [Order] = :orderType
        AND [Posting Date] BETWEEN :fyStart AND :fyEnd";
...
$fcCount += fetchCount($db, $site, 'fc_oil_change', 'FC_OIL_CHANGE ORDER', true, $financialYearStart007, $financialYearEnd007);
$fcCount += fetchCount($db, $site, 'dispute', 'FC_OIL_CHANGE ORDER', true, $financialYearStart007, $financialYearEnd007);
...
$gbCount += fetchCount($db, $site, 'gb_oil_change', 'GB_OIL_CHANGE ORDER', true, $financialYearStart007, $financialYearEnd007);
$gbCount += fetchCount($db, $site, 'dispute', 'GB_OIL_CHANGE ORDER', true, $financialYearStart007, $financialYearEnd007);
...
$pdCount += fetchCount($db, $site, 'PD_OIL_CHG_ORDER', 'PD_OIL_CHG_ORDER', true, $financialYearStart007, $financialYearEnd007);
$pdCount += fetchCount($db, $site, 'dispute', 'PD_OIL_CHG_ORDER', true, $financialYearStart007, $financialYearEnd007);
...
$ydCount += fetchCount($db, $site, 'YD_OIL_CHG_ORDER', 'YD_OIL_CHG_ORDER', true, $financialYearStart007, $financialYearEnd007);
$ydCount += fetchCount($db, $site, 'dispute', 'YD_OIL_CHG_ORDER', true, $financialYearStart007, $financialYearEnd007);
    ...
    $fileName = "Teco_Pending_Report_{$financialYearLabel007}.xlsx";
...
$subject = "FW: GENTLE REMINDER 1 - PENDING TECO - IF PHYSICALLY GB FC YDPD OIL CHANGE WAS DONE - CLOSE IT BEFORE DT $currentDate007 WITHOUT FAIL";

$body = "Respective AIC, SI & PMO,<br><br>
It has been observed that the physically oil change was done at location, but the SAP activity is pending due to that Actual status of oil change was not displayed in front of management.<br><br>
To avoid that please find the attached file containing suspect location where the TECO was pending.<br><br>
Kindly do the TECO of oil change order before $currentDate007 by end of the day if oil change was done & confirm on mail.<br><br>
NOTE – If you found an error during SAP – TECO process then write a mail with error snapshot 
to Mr Rahul Raut & Mr Harshvardhan at rahul.raut@suzlon.com & sbatech17@suzlon.com.<br><br>
1. GI DONE.<br>2. USED OIL RETURN TO SYSTEM<br>3. SAP OIL CHANGE PROCESS PENDING<br>4. GOODS MOVEMENT DONE";
